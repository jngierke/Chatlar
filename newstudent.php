...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
	...
	if (empty($_POST['email'])) {
		$errors['email'] = 'Oops! You forgot something...';
	} 
	...
	if (empty($errors)) {
		...
		mysql_query("INSERT INTO users SET first_name = '$first_name', last_name = '$last_name', email = '$email', username = '$username', password = '$password'") or die('Query failed: ' . mysql_error());
		
		// redirect user to another page and exit
		header("Location: checkyourself.php");
		exit();
	}
...
<form name="new_user" method="post">
		...
		<div class="form-group">
			<label for="username">Chatlar Name (as appears in chat)</label>
		...
		<div class="form-group">
  		...
  		<div class="form-group">
  			<button type="submit" name="Submit" value="submit" class="btn btn-default">¡Vamos!</button>
		</div>		
	</form>
